...
class Agent(object):
    ...
    def reset(self, px=None, py=None, gx=None, gy=None, pref_speed=None, radius=None, heading=None, previous_history=None , start_step_num=None, start_t=None):
        ...
        :param previous_history: in case agent already in motion before simulation starts
        ...
        #This variable governs the history length of this agent
        self.history_len_multiplier = 3000 #1.2 
        self.num_states_in_history = int(self.history_len_multiplier*self.time_remaining_to_reach_goal / self.dt_nominal)
    ...
    def cal_state(self, px, py, vx=None, vy=None, heading=None, move=None):
        """ Without doing a full reset, update the agent's current state (pos, vel, heading).
        ...
        if vx is None or vy is None:
            if self.step_num == 0:
                # On first timestep, just set to zero
                vel_global_frame = np.array([0,0])
            else:
                # Interpolate velocity from last pos
                vel_global_frame = (np.array([px, py]) - self.pos_global_frame) / self.dt_nominal
        else:
            vel_global_frame = np.array([vx, vy])

        if heading is None:
            # Estimate heading to be the direction of the velocity vector
            heading = np.arctan2(vel_global_frame[1], vel_global_frame[0])
            delta_heading_global_frame = wrap(heading - self.heading_global_frame)
        else:
            delta_heading_global_frame = wrap(heading - self.heading_global_frame)

        pos_global_frame = np.array([px, py])
        speed_global_frame = np.linalg.norm(vel_global_frame)
        heading_global_frame = heading

        return speed_global_frame, delta_heading_global_frame
    ...
    def take_action(self, action, dt):
        ...
        if self.is_at_goal or self.ran_out_of_time or self.is_out_of_bounds:
            ...
            #Added for output complete dataset
            #Keep outputing position in history, otherwise in the dataset, once any conditions above were reached, this agent will just disappear in the output dataset
            self._update_state_history()
